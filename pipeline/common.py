...
fallback, deterministic browser-parseable JSON) are applied uniformly. Mirrors
the house template (see SETUP.md §7).
"""
...
def fetch_text(session: requests.Session, url: str, dest: Path,
    ...
    On a failed/empty fetch the existing cached file is kept (never overwritten)
    and its content returned; if nothing cached and offline, returns None.
    """
    if dest.exists() and (offline or os.environ.get("GREENIUM_USE_CACHE") == "1"):
...
def write_json(path: Path, obj, validate: bool = True) -> None:
    ...
    if validate:
        json.loads(text, parse_constant=_reject_nonfinite)  # the exact CI guard
...
def data_mode() -> dict:
    p = FACTS / "data_mode.json"
    if p.exists():
        return read_json(p)
    return {"data_mode": "live"}


def maturity_years(maturity, asof) -> float:
    """Years to maturity (actual/365.25) from `asof` date."""
